...
func main() {
	...
	// Get config
	conf := &conf{}
	flag.StringVar(&conf.TLSCertPath, "cert-path", "./server.crt", "The path to the PEM-encoded TLS certificate")
	flag.StringVar(&conf.TLSKeyPath, "key-path", "./server.key", "The path to the unencrypted TLS key")
	...
	flag.StringVar(&conf.Host, "host", "admission.auto.delete", "The hostname for the service")
	flag.Parse()
	...
	admissions.Handle("/auto-delete", &admissioncontrol.AdmissionHandler{
		AdmitFunc: autodelete.AutoDelete(nil),
		Logger:    logger,
